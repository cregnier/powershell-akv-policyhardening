...
# Display environment-specific behavior
if ($Environment -eq 'DevTest') {
    Write-Host "`n🧪 DEV/TEST MODE - What Gets Created:" -ForegroundColor Cyan
    Write-Host "  ✅ Policy-required infrastructure:" -ForegroundColor Green
    Write-Host "     • Managed Identity (for 8 auto-remediation policies)" -ForegroundColor White
    Write-Host "     • Event Hub namespace (for diagnostic logs)" -ForegroundColor White
    Write-Host "     • Log Analytics workspace (for monitoring)" -ForegroundColor White
    Write-Host "     • VNet + Subnet (for private endpoints)" -ForegroundColor White
    Write-Host "     • Private DNS Zone (for private Key Vaults)" -ForegroundColor White
    Write-Host "  ✅ Test environment:" -ForegroundColor Green
    Write-Host "     • 3 Key Vaults (compliant, partial, non-compliant)" -ForegroundColor White
    Write-Host "     • Test data (secrets, keys, certificates)" -ForegroundColor White
    Write-Host "  ℹ️  Purpose: Complete testing in isolated environment" -ForegroundColor Gray
} else {
    Write-Host "`n🏭 PRODUCTION MODE - What Gets Created:" -ForegroundColor Cyan
    Write-Host "  ✅ ONLY policy-required infrastructure:" -ForegroundColor Green
    Write-Host "     • Managed Identity (for 8 auto-remediation policies)" -ForegroundColor White
    Write-Host "     • Event Hub namespace (for diagnostic log policies)" -ForegroundColor White
    Write-Host "     • Log Analytics workspace (for monitoring policies)" -ForegroundColor White
    Write-Host "  ❌ What does NOT get created:" -ForegroundColor Red
    Write-Host "     • Key Vaults (policies monitor YOUR EXISTING vaults)" -ForegroundColor White
    Write-Host "     • Test data (uses YOUR existing secrets/keys/certificates)" -ForegroundColor White
    Write-Host "     • VNet/Subnet (optional - only if using private endpoints)" -ForegroundColor White
    Write-Host "  ℹ️  Purpose: Minimal infrastructure for monitoring existing resources" -ForegroundColor Gray
}
...
Write-Host "`n📚 Documentation:" -ForegroundColor $colors.Highlight
Write-Host "  • Quick Start Guide: See QUICKSTART.md for all deployment scenarios" -ForegroundColor Cyan
Write-Host "  • Detailed Workflows: See DEPLOYMENT-WORKFLOW-GUIDE.md for step-by-step procedures" -ForegroundColor Cyan
Write-Host "  • Infrastructure Details: See DEPLOYMENT-PREREQUISITES.md for requirements" -ForegroundColor Cyan
Write-Host "  • Cleanup Procedures: See CLEANUP-EVERYTHING-GUIDE.md when ready to remove resources" -ForegroundColor Cyan
