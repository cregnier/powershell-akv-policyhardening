...
function Get-TargetSubscriptions {
    <#
    .SYNOPSIS
        Get target subscriptions based on SubscriptionMode parameter.
    .DESCRIPTION
        Returns array of subscription objects based on mode: Current, All, Select (interactive), or CSV (file-based).
    .PARAMETER Mode
        Subscription targeting mode: Current, All, Select, CSV
    .PARAMETER CSVPath
        Path to CSV file with SubscriptionId column (required if Mode=CSV)
    #>
    param(
        [Parameter(Mandatory)]
    ...
    Write-Log "Multi-Subscription Mode: $Mode" -Level 'INFO'
    
    switch ($Mode) {
        'Current' {
            $ctx = Get-AzContext
            if (-not $ctx -or -not $ctx.Subscription) {
                Write-Log -Message 'No subscription context found. Please login and select a subscription.' -Level 'ERROR'
                throw 'No Azure context'
            }
            Write-Log "Using current subscription: $($ctx.Subscription.Name) ($($ctx.Subscription.Id))" -Level 'SUCCESS'
            return @($ctx.Subscription)
        }
        
        'All' {
            Write-Host "`n⚠️  WARNING: Deploying to ALL subscriptions in tenant" -ForegroundColor Yellow
            $allSubs = Get-AzSubscription | Where-Object { $_.State -eq 'Enabled' }
            Write-Host "Found $($allSubs.Count) enabled subscriptions:" -ForegroundColor Cyan
            $allSubs | Select-Object Name, Id, State | Format-Table -AutoSize
            
            Write-Host "`nThis will deploy policies to ALL $($allSubs.Count) subscriptions." -ForegroundColor Red
            $confirm = Read-Host "Type 'ALL' to confirm deployment to all subscriptions"
            ...
            Write-Log "Deploying to $($allSubs.Count) subscriptions" -Level 'SUCCESS'
            return $allSubs
        }
        
        'Select' {
            $allSubs = Get-AzSubscription | Where-Object { $_.State -eq 'Enabled' }
            Write-Host "`nAvailable Subscriptions (Enabled):" -ForegroundColor Cyan
            
            for ($i = 0; $i -lt $allSubs.Count; $i++) {
                Write-Host "[$i] $($allSubs[$i].Name) - $($allSubs[$i].Id)" -ForegroundColor Gray
            }
            
            Write-Host "`nEnter subscription numbers separated by commas (e.g., 0,2,5)" -ForegroundColor Yellow
            Write-Host "Or type 'all' to select all subscriptions" -ForegroundColor Yellow
            $selection = Read-Host "Selection"
            
            if ($selection.ToLower() -eq 'all') {
                Write-Log "Selected all $($allSubs.Count) subscriptions" -Level 'SUCCESS'
                return $allSubs
            }
            
            $indices = $selection -split ',' | ForEach-Object { $_.Trim() } | Where-Object { $_ -match '^\d+$' } | ForEach-Object { [int]$_ }
            $selectedSubs = $indices | ForEach-Object {
                if ($_ -ge 0 -and $_ -lt $allSubs.Count) {
                    $allSubs[$_]
                } else {
                    Write-Log "Invalid index: $_" -Level 'WARN'
                }
            }
            
            if ($selectedSubs.Count -eq 0) {
                Write-Log "No valid subscriptions selected" -Level 'ERROR'
                throw 'No subscriptions selected'
            }
            
            Write-Host "`nSelected $($selectedSubs.Count) subscription(s):" -ForegroundColor Green
            $selectedSubs | Select-Object Name, Id | Format-Table -AutoSize
        ...
        'CSV' {
            if (-not $CSVPath) {
                Write-Log "CSV mode requires -SubscriptionCSV parameter" -Level 'ERROR'
                throw 'Missing SubscriptionCSV parameter'
            }
            ...
            # Validate CSV has SubscriptionId column
            if (-not ($csvData[0].PSObject.Properties.Name -contains 'SubscriptionId')) {
                Write-Log "CSV must have 'SubscriptionId' column" -Level 'ERROR'
                throw "CSV missing SubscriptionId column"
            }
            
            $subIds = $csvData | Select-Object -ExpandProperty SubscriptionId | Where-Object { $_ }
            
            if ($subIds.Count -eq 0) {
                Write-Log "No subscription IDs found in CSV" -Level 'ERROR'
                throw 'No subscription IDs in CSV'
            }
            
            Write-Host "`nLoading $($subIds.Count) subscription(s) from CSV..." -ForegroundColor Cyan
            ...
            foreach ($subId in $subIds) {
                try {
                    ...
                    if ($sub.State -eq 'Enabled') {
                        $selectedSubs += $sub
                        Write-Host "  ✓ $($sub.Name) ($($sub.Id))" -ForegroundColor Green
                    } else {
                        Write-Log "Skipping disabled subscription: $($sub.Name)" -Level 'WARN'
                    }
                } catch {
                    Write-Log "Subscription not found or no access: $subId" -Level 'WARN'
                }
            }
            
            if ($selectedSubs.Count -eq 0) {
                Write-Log "No valid enabled subscriptions found from CSV" -Level 'ERROR'
                throw 'No valid subscriptions'
            }
            
            Write-Log "Loaded $($selectedSubs.Count) enabled subscription(s) from CSV" -Level 'SUCCESS'
            return $selectedSubs
        }
    }
}
...
function Remove-KeyVaultPolicyAssignments {
    ...
    try {
        ...
        Write-Host ""
        Write-Host "📚 Next: See CLEANUP-EVERYTHING-GUIDE.md for infrastructure cleanup" -ForegroundColor Cyan
        Write-Host "📚 Redeploy: See QUICKSTART.md for deployment scenarios" -ForegroundColor Cyan
    }
    catch {
...
function Main {
    ...
    param(
        ...
        [ValidateSet('Current', 'All', 'Select', 'CSV')]
        [string]$SubscriptionMode = 'Current',  # Subscription targeting mode: Current (default), All, Select (interactive), CSV (file-based)
        [string]$SubscriptionCSV,  # Path to CSV file with subscription IDs (requires SubscriptionMode=CSV)
        
        # Dry-run / WhatIf
        [switch]$WhatIf,  # Preview what would be deployed without making changes
    ...
    if ($CheckCompliance) {
        ...
        Write-Host ""
        Write-Host "📚 Next Steps: See QUICKSTART.md for next deployment scenarios" -ForegroundColor Cyan
        Write-Host "📚 Cleanup: See CLEANUP-EVERYTHING-GUIDE.md for cleanup procedures" -ForegroundColor Cyan
        Write-Host ""
    ...
    if ($WhatIf) {
        ...
        Write-Host "`nThis is a dry-run. The script will show what WOULD happen without making any changes." -ForegroundColor Yellow
        Write-Host "To execute the deployment for real, remove the -WhatIf parameter.`n" -ForegroundColor Yellow
    }

    # Multi-Subscription Support: Get target subscriptions
    $targetSubscriptions = Get-TargetSubscriptions -Mode $SubscriptionMode -CSVPath $SubscriptionCSV
    
    if ($targetSubscriptions.Count -gt 1) {
        Write-Host "`n╔══════════════════════════════════════════════════════════════╗" -ForegroundColor Cyan
        Write-Host "║  🌐 MULTI-SUBSCRIPTION DEPLOYMENT MODE                      ║" -ForegroundColor Cyan
        Write-Host "╚══════════════════════════════════════════════════════════════╝" -ForegroundColor Cyan
        Write-Host "`nDeploying to $($targetSubscriptions.Count) subscriptions" -ForegroundColor Green
        Write-Host "Parameter File: $ParameterOverridesPath" -ForegroundColor Gray
        Write-Host "Policy Mode: $(if ($PolicyMode) { $PolicyMode } else { 'Will be determined per subscription' })" -ForegroundColor Gray
    }
    
    # Multi-subscription results tracking
    $multiSubResults = @()
    
    # Loop through each target subscription
    foreach ($targetSub in $targetSubscriptions) {
        # Display subscription header for multi-sub deployments
        if ($targetSubscriptions.Count -gt 1) {
            Write-Host "`n╔══════════════════════════════════════════════════════════════╗" -ForegroundColor Magenta
            Write-Host "║  📍 Subscription: $($targetSub.Name)" -ForegroundColor Magenta  
            Write-Host "║  🆔 ID: $($targetSub.Id)" -ForegroundColor Magenta
            Write-Host "╚══════════════════════════════════════════════════════════════╝" -ForegroundColor Magenta
            
            # Switch to target subscription
            try {
                Set-AzContext -Subscription $targetSub.Id -ErrorAction Stop | Out-Null
                Write-Log "Switched to subscription: $($targetSub.Name) ($($targetSub.Id))" -Level 'SUCCESS'
            } catch {
                Write-Log "Failed to switch to subscription $($targetSub.Name): $($_.Exception.Message)" -Level 'ERROR'
                $multiSubResults += @{
                    Subscription = $targetSub.Name
                    SubscriptionId = $targetSub.Id
                    Status = 'Failed'
                    Error = "Context switch failed: $($_.Exception.Message)"
                    PoliciesDeployed = 0
                }
                continue
            }
    ...
    foreach ($n in $names) {
        ...
        $res = Assign-Policy -DisplayName $n -Scope $scope -Mode $selectedMode -DryRun:$DryRun -WhatIf:$WhatIf -ParameterOverrides $overrides -Mapping $policyMapping -MaxRetries $MaxRetries -IdentityResourceId $IdentityResourceId
        $assignResults += $res
    }
    ...
    # Store subscription results for multi-sub deployments
    if ($targetSubscriptions.Count -gt 1) {
        $successCount = ($assignResults | Where-Object { $_.Status -in @('Assigned', 'Updated') }).Count
        $multiSubResults += @{
            Subscription = $targetSub.Name
            SubscriptionId = $targetSub.Id  
            Status = 'Success'
            PoliciesDeployed = $successCount
            CompliancePercent = if ($compliance -and $compliance.OperationalStatus) { $compliance.OperationalStatus.OverallCompliancePercent } else { 'N/A' }
            HtmlReport = $htmlReport
        }
    }
        
    } catch {
        # Subscription-level error handling for multi-sub deployments
        if ($targetSubscriptions.Count -gt 1) {
            Write-Log "Subscription deployment failed for $($targetSub.Name): $($_.Exception.Message)" -Level 'ERROR'
            $multiSubResults += @{
                Subscription = $targetSub.Name
                SubscriptionId = $targetSub.Id
                Status = 'Failed'
            ...
            # Single subscription mode - rethrow error
            throw
        }
    }
    } # End of subscription loop
    
    # Display multi-subscription aggregate results
    if ($targetSubscriptions.Count -gt 1) {
        Write-Host "`n╔══════════════════════════════════════════════════════════════╗" -ForegroundColor Green
        Write-Host "║  🌐 MULTI-SUBSCRIPTION DEPLOYMENT SUMMARY                   ║" -ForegroundColor Green
        Write-Host "╚══════════════════════════════════════════════════════════════╝" -ForegroundColor Green
        
        Write-Host "`nDeployment Results Across $($targetSubscriptions.Count) Subscription(s):`n" -ForegroundColor Cyan
        
        $multiSubResults | Format-Table -Property `
            @{Label='Subscription';Expression={$_.Subscription}},
            @{Label='Status';Expression={$_.Status}},
            @{Label='Policies';Expression={$_.PoliciesDeployed}},
            @{Label='Compliance %';Expression={$_.CompliancePercent}},
            @{Label='Error';Expression={if ($_.Error) { $_.Error.Substring(0, [Math]::Min(50, $_.Error.Length)) + '...' } else { '-' }}} `
            -AutoSize
        
        $successSubs = ($multiSubResults | Where-Object { $_.Status -eq 'Success' }).Count
        $failedSubs = ($multiSubResults | Where-Object { $_.Status -eq 'Failed' }).Count
        
        Write-Host "`n📊 Overall Results:" -ForegroundColor Cyan
        Write-Host "  ✅ Successful: $successSubs of $($targetSubscriptions.Count) subscriptions" -ForegroundColor Green
        if ($failedSubs -gt 0) {
            Write-Host "  ❌ Failed: $failedSubs of $($targetSubscriptions.Count) subscriptions" -ForegroundColor Red
        }
        
        Write-Host "`n📄 Individual Reports:" -ForegroundColor Cyan
        foreach ($result in $multiSubResults | Where-Object { $_.HtmlReport }) {
            Write-Host "  • $($result.Subscription): $($result.HtmlReport)" -ForegroundColor Gray
        }
        Write-Host ""
    }
